...
library Verify {
	...
	function isValidSignatureForHash(uint256 PK_x, uint256 PK_y, uint256 r, uint256 s, uint256 messageHash) public pure returns (bool valid) {
		require(r >= 1);
		require(s >= 1);
		require(r < n);
		require(s < n);
	...
	function isValidSignatureForMessage(uint256 PK_x, uint256 PK_y, uint256 r, uint256 s, string memory message) public pure returns (bool valid) {
